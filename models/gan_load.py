...
class StyleGAN2Wrapper(nn.Module):
    ...
    def forward(self, input, input_is_latent=True, noise=None):
        image, _ = self.style_gan2([input], input_is_latent=input_is_latent, noise=None)
        return image

    def gen_shifted(self, z, shift, noises=None):
        if self.shift_in_w:
            w = self.style_gan2.get_latent(z)
            return self.forward(w + shift, input_is_latent=True, noise=noises)
        else:
            return self.forward(z + shift, input_is_latent=False, noise=noises)
...
def make_style_gan2(size, weights, shift_in_w=True):
    ...
    # G.load_state_dict(torch.load(weights, strict=False)['g_ema'])
